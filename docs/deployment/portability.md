...
| 能力 | 命令 | 用途 | 缺失时 |
|---|---|---|---|
| Docker | `docker info` | 全部容器 | `init.sh` 报错并退出 |
| nftables | `nft` | 赛事隔离防火墙 | `init.sh` 报错并退出 |
| WireGuard | `wg` | 选手接入隧道 | `init.sh` 报错并退出 |
| iproute2 | `ip` | 网络接口/路由 | `init.sh` 报错并退出 |
| sysctl | `sysctl` | 内核参数 | `init.sh` 报错并退出 |
| modprobe | `modprobe` | 内核模块（br_netfilter） | `init.sh` 报错并退出 |

`init.sh` **按功能检测**而非按发行版包名：提供这些命令的主机即可初始化，不假设
“Arch 有什么包”。已知发行版安装命令作为提示（Arch `pacman -S`），Debian/Fedora
未硬编码（避免盲装）。

## 3. 必需内核参数与模块（init.sh 自动持久化）
...
`init.sh` 同时做运行时 `sysctl -w` / `modprobe`（幂等），并持久化到
...
- `init.sh` 用 `runuser -u floatctf -- docker info` 验证组真实生效。
...
所有端口可经 `/home/floatctf/.env`（或环境变量）调整，`deploy.sh` 部署前检测冲突。

## 7. 迁移（porting）到另一台主机

1. 新主机：`sudo scripts/init.sh`（建用户/布局/内核参数/权限）。
2. 构建/拷贝 release 包（`scripts/build-release.sh` → `release/floatctf-*`）。
3. `FCTF_ROOT=/新的/home/floatctf scripts/deploy.sh <release-dir>`（首次部署生成密钥）。
4. 数据迁移：`data/postgres/` 与 `data/rustfs/` 物理拷贝（原主机停容器后拷贝
