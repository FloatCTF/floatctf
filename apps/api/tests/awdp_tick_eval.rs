//! AWDP 回合 + 官方评估集成测试（DB + Docker gated）。
//!
...
use floatctf::modules::event::awdp::{
    domain::AwdpConfig,
    repo::{evaluation_repo, event_gamebox_repo, event_repo, score_repo},
    service::{
        evaluation,
...
/// 建已开始（Break）的事件 + 完整 [awdp] GameBox（自定义 exploit 语义）。
async fn seed_event_and_gamebox(
    ...
    event_repo::ensure_by_event_id(db, event.id, &AwdpConfig::default())
        .await
        .unwrap();
    // 直接推进到 Break（tick 的 pending→break 单独测试）。
...
/// 打开指定 sequence 的 round 窗口（starts 过去 / cutoff 未来），事件 next_action_at 指向其 cutoff。
/// 用于 patch 提交前的 open round 窗口。
async fn open_round(db: &sea_orm::DatabaseConnection, event_id: Uuid, sequence: i32) {
    let now = chrono::Utc::now();
    ...
    let row = event_repo::require_by_event_id(db, event_id).await.unwrap();
    let mut am: floatctf::entity::awdp_events::ActiveModel = row.into();
    am.next_action_at = Set(Some((now + chrono::Duration::minutes(10)).into()));
    am.updated_at = Set(now.into());
    am.update(db).await.unwrap();
}

/// 把指定 sequence 的 round 拨到过去（其余 round 拨到未来），并让事件 due。
/// 保证任意时刻只有一个 open round（patch 需要 open round）。
async fn expire_round(db: &sea_orm::DatabaseConnection, event_id: Uuid, sequence: i32) {
    let now = chrono::Utc::now();
    for row in awdp_fix_rounds::Entity::find()
        .filter(awdp_fix_rounds::Column::EventId.eq(event_id))
        .all(db)
    ...
    // tick 以 awdp_events.next_action_at 领取事件。
    let row = event_repo::require_by_event_id(db, event_id).await.unwrap();
    let mut am: floatctf::entity::awdp_events::ActiveModel = row.into();
...
async fn tick_driven_rounds_and_scoring() {
    ...
    let (ev_a, eg_a) = seed_event_and_gamebox(&db, "fA", EXPLOIT_ALWAYS_SUCCESS).await;
    // 事件 B：exploit 总是失败 → PATCHED +score。
    let (ev_b, eg_b) = seed_event_and_gamebox(&db, "fB", EXPLOIT_ALWAYS_FAIL).await;
    ...
    let inst_a = runtime::start_instance(&db, &docker, JWT_SECRET, ev_a, eg_a, sub_a, "flag")
        .await
        .expect("start A");
    let inst_b = runtime::start_instance(&db, &docker, JWT_SECRET, ev_b, eg_b, sub_b, "flag")
        .await
        .expect("start B");

    // 1. tick：Break 到期 → Fix + rounds 物化。
    tick_service::tick_once(&db, &docker, JWT_SECRET)
        .await
        .expect("tick");
    let row_a = event_repo::require_by_event_id(&db, ev_a).await.unwrap();
    assert_eq!(row_a.phase, AwdpPhase::Fix);
    let rounds = floatctf::modules::event::awdp::repo::round_repo::list_for_event(&db, ev_a)
        .await
        .unwrap();
    ...
    expire_round(&db, ev_a, 1).await;
    expire_round(&db, ev_b, 1).await;
    tick_service::tick_once(&db, &docker, JWT_SECRET)
        .await
        .expect("tick r1");
    ...
    let evals_a = evaluation_repo::list_for_event(&db, ev_a).await.unwrap();
    let e1 = evals_a
        .iter()
    ...
    let score_a = score_repo::my_total(&db, ev_a, Some(user_a), None)
        .await
        .unwrap();
    assert_eq!(score_a, 0, "NO_PATCH +0");

    // 3. Round 2：先打开 round 2 窗口，再应用 patch（事件 A 需要真实 APPLIED 才 eligible）。
    open_round(&db, ev_a, 2).await;
    open_round(&db, ev_b, 2).await;
    ...
    expire_round(&db, ev_a, 2).await;
    expire_round(&db, ev_b, 2).await;
    tick_service::tick_once(&db, &docker, JWT_SECRET)
        .await
        .expect("tick r2a");
    tick_service::tick_once(&db, &docker, JWT_SECRET)
    ...
    let evals_b = evaluation_repo::list_for_event(&db, ev_b).await.unwrap();
    let r2_b = evals_b
        .iter()
        .filter(|e| e.status != AwdpEvaluationStatus::NoPatch)
        .min_by_key(|e| e.created_at)
    ...
    let row_b = event_repo::require_by_event_id(&db, ev_b).await.unwrap();
    let score_b = score_repo::my_total(&db, ev_b, Some(user_b), None)
        .await
        .unwrap();
    ...
    let score_b2 = score_repo::my_total(&db, ev_b, Some(user_b), None)
        .await
        .unwrap();
    ...
    for seq in 3..=6 {
        expire_round(&db, ev_a, seq).await;
        expire_round(&db, ev_b, seq).await;
        tick_service::tick_once(&db, &docker, JWT_SECRET)
            .await
            .expect("tick seq");
        loop {
...
async fn tick_starts_pending_event_at_start_time() {
    ...
    // 事件 start_time 在过去、未手动开始 → tick 自动进入 Break。
    let base = chrono::Utc::now();
    let event = events::ActiveModel {
        id: Set(Uuid::new_v4()),
        family: Set(EventFamily::Awdp),
    ...
    event_repo::ensure_by_event_id(&db, event.id, &AwdpConfig::default())
        .await
        .unwrap();
    let row = event_repo::require_by_event_id(&db, event.id)
        .await
        .unwrap();
    assert!(row.next_action_at.is_some(), "ensure 时排定 start");
    ...
    let row = event_repo::require_by_event_id(&db, event.id)
        .await
        .unwrap();
    assert_eq!(row.phase, AwdpPhase::Break, "pending + start due → Break");
