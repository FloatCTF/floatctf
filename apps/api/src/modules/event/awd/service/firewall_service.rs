...
pub async fn build_desired_state<C: ConnectionTrait + Send>(
    ...
    let event_networks = crate::modules::event::awd::repo::event_network_repo::list_all(db).await?;
    ...
    for event in events {
        ...
        desired.events.push(DesiredEventPolicy {
            event_key:
                crate::modules::event::awd::infrastructure::firewall::NftObjectName::event_key(
                    &event.event_id,
                )
                .as_str()
                .to_string(),
...
pub async fn reconcile_global(
    ...
    revision: u64,
) -> AwdResult<crate::modules::event::awd::infrastructure::firewall::FirewallApplyResult> {
...
pub async fn flush_event_connections(
    network: &dyn crate::modules::event::awd::infrastructure::network::AwdNetworkRuntime,
    event_id: Uuid,
    gamebox_cidr: &str,
) {
    use crate::modules::event::awd::infrastructure::network::EventNetworkIdentity;
