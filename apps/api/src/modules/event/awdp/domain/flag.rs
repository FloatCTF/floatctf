...
//!
//! Flag 稳定绑定：event × event_gamebox × participant（user/team）。
//! - 每 participant 的容器拿到自己的 flag（不能提交他人 flag）；
//! - 跨 reset 稳定（同 subject 同 gamebox 永远同 flag）；
//! - Break 不需要 round rotating flag。
//!
//! 密钥派生：平台级 Secret（`auth.jwt_secret`，仅内存）经 HKDF 派生 per-event 密钥，
...
/// 用平台 Secret 派生 per-event flag HMAC 密钥（HKDF-SHA256，32 字节）。
fn derive_event_flag_key(jwt_secret: &[u8], event_id: &str) -> Vec<u8> {
...
/// 生成确定性 Break flag：`{prefix}{awdp:{event}:{eg}:{subject} 的 HMAC 前 16 字节}`
pub fn awdp_flag(
    jwt_secret: &[u8],
    event_id: Uuid,
    ...
    let key = derive_event_flag_key(jwt_secret, &event_id.to_string());
...
mod tests {
    ...
    fn flag_is_deterministic_per_subject() {
        ...
        let f1 = awdp_flag(secret, e, g, Some(u), None, "flag");
        let f2 = awdp_flag(secret, e, g, Some(u), None, "flag");
        assert_eq!(f1, f2);
    ...
    fn different_subjects_get_different_flags() {
        ...
        let secret = b"platform-secret";
        let f1 = awdp_flag(secret, e, g, Some(u1), None, "flag");
        let f2 = awdp_flag(secret, e, g, Some(u2), None, "flag");
        assert_ne!(f1, f2);
        // user vs team subject 也不同
        let t = Uuid::from_str("00000000-0000-0000-0000-00000000000c").unwrap();
        let f3 = awdp_flag(secret, e, g, None, Some(t), "flag");
        assert_ne!(f1, f3);
    }

    #[test]
    fn different_events_get_different_flags() {
        let (e, g, u) = ids();
        let e2 = Uuid::from_str("00000000-0000-0000-0000-0000000000ff").unwrap();
        let secret = b"platform-secret";
        assert_ne!(
            awdp_flag(secret, e, g, Some(u), None, "flag"),
            awdp_flag(secret, e2, g, Some(u), None, "flag")
        );
    }
