...
pub async fn run_precheck(
    ...
    let event_gameboxes =
        crate::modules::event::awd::repo::event_gamebox_repo::find_event_gameboxes_by_event(
            db, event_id,
        )
        .await
        .map_err(|e| AwdError::Database(e.to_string()))?;
    ...
    if event_gameboxes.is_empty() {
        ...
        for eg in &event_gameboxes {
            match crate::modules::event::awd::service::gamebox_service::resolve_event_gamebox_spec(
                db, eg.id,
            )
            .await
            {
...
async fn check_wireguard(
    ...
    CheckReport,
    Option<crate::modules::event::awd::infrastructure::network::NetworkObservedState>,
    usize,
) {
    let mut report = CheckReport::default();
...
mod tests {
    use super::*;
    use crate::modules::event::awd::infrastructure::firewall::NoopFirewallRuntime;
