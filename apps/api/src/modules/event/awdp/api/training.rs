...
pub async fn upload_patch(
    ...
    log_practice_action(
        ...
        user.id,
        "awdp.train.patch",
        json!({ "run_id": run.id, "gamebox_id": gamebox_id, "status": status, "error_message": error_message }),
    )
    .await;
    UniResponse::ok(
        PatchSubmitResponse {
...
pub async fn run_stream(
    ...
    let body = unfold(
        (rx, false, run_id, keepalive_interval),
        |(mut rx, primed, run_id, keepalive_interval)| async move {
            if !primed {
                return Some((
                    Ok::<_, actix_web::Error>(web::Bytes::from(": connected\n\n")),
                    (rx, true, run_id, keepalive_interval),
                ));
            }
            loop {
                tokio::select! {
